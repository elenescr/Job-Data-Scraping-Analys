...
def scrape_remotive_jobs(limit=1000):
    options = Options()
    ...
    try:
        ...
        for card in job_cards[:limit]:
            try:
                bolds = card.find_elements(By.CSS_SELECTOR, "span.remotive-bold")
                title = bolds[0].text if bolds else "Unknown"
                company = bolds[-1].text if len(bolds) > 1 else "Unknown"
            except:
                title = "Unknown"
                company = "Unknown"

            try:
                location = card.find_element(By.CLASS_NAME, "location-text").text
            except:
                location = "Remote"
